import { baseUrlHint, type ErrorContext } from "./error-hints.ts";

/**
 * Backend hata kodunun kullanıcı diline çevrilmesi.
 *
 * `errors.ts`'ten AYRI bir modül — `error-hints.ts` ile aynı gerekçe: o dosya
 * `ApiError` sınıfını içeri alıyor, o da `api.ts` üzerinden TypeScript
 * parameter property kullanan koda bağlanıyor ve Node'un tip soyma kipi bunu
 * desteklemediği için `node --test` altında yüklenemiyor.
 *
 * Ürünün her hata mesajı buradan geçtiği için test edilebilir kalması gerek:
 * yanlış çeviri sessiz bir arıza, kullanıcı ekranda bir cümle görür ve o
 * cümlenin yanlış işi önerdiğini anlayamaz.
 *
...
export function describeApiError(
  ...
  switch (code) {
    ...
    case "invalid_base_url":
      return { message, hint: baseUrlHint(context) };
    case "bad_catalog":
      return {
    ...
    default:
      // Backend'in mesajı OLDUĞU GİBİ geçer. Yutulup "bir hata oluştu"ya
      // çevrilseydi, backend'de özenle yazılan her yeni hata mesajı arayüzde
      // sessizce kaybolurdu.
      return { message };
  }
}
